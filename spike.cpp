...
{
    // Load the Spike png image
    SDL_Surface* surface = IMG_Load("assets/Spike.png");
    if (surface == NULL) 
    {
        std::cout << "Could not load spike image: " << IMG_GetError() << std::endl;
        exit(EXIT_FAILURE);
    }
...
void renderSpike(SDL_Renderer* renderer, SDL_Texture* spikeTexture, int x, int y, int width, int height)
{
    ...
    SDL_RenderCopy(renderer, spikeTexture, &srcRect, &destRect);
}